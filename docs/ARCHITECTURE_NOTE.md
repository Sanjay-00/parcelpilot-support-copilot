...
- **`thinking_config: {thinking_budget: 0}` on every Gemini call, measured, not assumed.** Every call this system makes is classification, extraction, relevance selection, or phrasing an already-computed decision -- none of it is multi-step reasoning, because that's exactly the job deterministic Python resolvers do instead. Measured directly against this project's own API key: an equivalent classification prompt used 245 total tokens with Gemini's default extended "thinking" versus 16 with it disabled -- roughly 15x, with no correctness difference (a full 132-test live-model run passed identically before and after, and wall-clock time for that run roughly halved too, since thinking adds latency as well as tokens). Turning this off everywhere is a direct consequence of the system's own trust boundary, not a quality trade-off made to save money.
